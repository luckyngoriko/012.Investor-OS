...
fn test_quantum_portfolio_optimization() {
    ...
    let result = optimizer
        .optimize_portfolio(&assets, &covariance, 0.5)
        .unwrap();
    ...
    println!(
        "   Expected Return: {:.2}%",
        result.expected_return * Decimal::from(100)
    );
...
fn test_federated_learning_round() {
    ...
    println!(
        ...
        result.participating_clients.len()
    );
...
fn test_neuromorphic_inference_engine() {
    ...
    println!(
        "   Energy Efficiency: {:.6} inferences/pJ",
        engine.energy_efficiency()
    );
...
fn test_regime_classification() {
    ...
    println!(
        "   Bullish data regime: {:?}",
        bullish_detector.current_regime()
    );
    println!(
        "   Bearish data regime: {:?}",
        bearish_detector.current_regime()
    );
...
fn test_regime_forecasting() {
    ...
    println!(
        ...
        forecast.expected_duration_days
    );
...
fn test_microstructure_analyzer() {
    ...
    book.bids
        .push(OrderBookLevel::new(Decimal::from(100), Decimal::from(5000)));
    book.bids
        .push(OrderBookLevel::new(Decimal::from(99), Decimal::from(3000)));
    book.asks
        .push(OrderBookLevel::new(Decimal::from(101), Decimal::from(4000)));
    book.asks
        .push(OrderBookLevel::new(Decimal::from(102), Decimal::from(2000)));
...
fn test_order_book_imbalance() {
    ...
    book.bids.push(OrderBookLevel::new(
        Decimal::from(100),
        Decimal::from(10000),
    ));
    book.asks
        .push(OrderBookLevel::new(Decimal::from(101), Decimal::from(2000)));
...
fn test_technology_readiness_levels() {
    ...
    for area in &areas {
        ...
        println!(
            "   {}: RL{} - {}",
            area.name, area.readiness_level, readiness
        );
    }
}
...
fn test_complete_experimental_pipeline() {
    ...
    let _quantum_result = quantum_optimizer
        .optimize_portfolio(&assets, &create_test_covariance(2), 0.5)
        .unwrap();
