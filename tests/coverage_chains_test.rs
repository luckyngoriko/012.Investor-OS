...
use investor_os::langchain::{
    ...
    chains::{SequentialChain, ParallelChain},
...
impl Chain for MockChain {
    async fn run(&self, ctx: ChainContext) -> Result<ChainResult, investor_os::langchain::ChainError> {
...
async fn test_sequential_chain() {
    let chain1 = MockChain { prefix: "A:".to_string() };
    let chain2 = MockChain { prefix: "B:".to_string() };
...
async fn test_parallel_chain() {
    let chain1 = MockChain { prefix: "Parallel1:".to_string() };
    let chain2 = MockChain { prefix: "Parallel2:".to_string() };
...
async fn test_sequential_chain_empty() {
    ...
    let ctx = ChainContext::new();
    let result = seq_chain.run(ctx).await;
    
    // Empty chain should return default/empty result
    assert!(result.is_ok());
}

#[tokio::test]
async fn test_chain_result_metadata() {
    let chain = MockChain { prefix: "".to_string() };
...
async fn test_prompt_template_with_history() {
    ...
    let history = vec![
        Message { role: Role::User, content: "Hello".to_string(), metadata: None },
        Message { role: Role::Assistant, content: "Hi there".to_string(), metadata: None },
    ];
