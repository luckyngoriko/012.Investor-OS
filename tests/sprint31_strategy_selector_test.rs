...
use investor_os::strategy_selector::{
    ...
    switcher::{SwitchConfig, StrategySwitcher},
    MarketIndicators, MarketRegime, RiskTolerance, Strategy, StrategySelectorEngine,
...
fn test_golden_path_strategy_selection() {
    ...
    let mean_reversion = create_strategy("MeanRev Plus", StrategyType::MeanReversion, 1.3, 0.12, 10000);
...
fn test_strategy_switching() {
    ...
    let selection = engine.select_strategy(MarketRegime::Trending, SelectionCriteria::default()).unwrap();
    
    // Execute switch
    let state = engine.execute_switch(
...
fn test_strategy_recommendations() {
    ...
    let conservative_recs = engine.get_recommendations(
        Decimal::from(10000),
    ...
    for rec in &conservative_recs {
        assert!(matches!(rec.risk_level, RiskLevel::VeryLow | RiskLevel::Low));
    }
    
    // Speculative investor can get all
    let speculative_recs = engine.get_recommendations(
        Decimal::from(10000),
...
fn test_market_regime_detection() {
    ...
    assert_eq!(engine.detect_regime(&volatile), MarketRegime::VolatilityExpansion);
