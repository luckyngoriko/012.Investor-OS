...
use investor_os::middleware::{RateLimiter, RateLimitResult};
...
fn test_k8s_manifests_exist() {
    ...
    for manifest in manifests {
        ...
        assert!(content.contains("apiVersion:"), "Invalid K8s manifest: {}", manifest);
    }
}
...
fn test_cicd_configuration() {
    ...
    assert!(content.contains("deploy-staging:"), "Missing staging deployment");
    assert!(content.contains("deploy-production:"), "Missing production deployment");
...
fn test_dockerfile_exists() {
    ...
    assert!(content.contains("FROM"), "Invalid Dockerfile - missing FROM");
...
fn test_env_example_exists() {
    ...
    assert!(content.contains("DATABASE_URL"), "Missing DATABASE_URL in example");
    assert!(content.contains("REDIS_URL"), "Missing REDIS_URL in example");
}
...
fn test_security_configuration() {
    ...
    assert!(content.contains("runAsNonRoot"), "Missing runAsNonRoot security context");
    assert!(content.contains("readOnlyRootFilesystem"), "Missing readOnlyRootFilesystem");
    assert!(content.contains("capabilities:"), "Missing capabilities drop");
}
