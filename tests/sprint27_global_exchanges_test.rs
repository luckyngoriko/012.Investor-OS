...
use investor_os::global::{
    GlobalExchangeRegistry, ExchangeId, Region,
    GlobalMarketData, GlobalQuote,
    free_data::{
        FreeDataAggregator, DataSource, FreeMarketData,
        CrossSourcePrice, CrossSourceValidation,
    },
};
...
fn test_exchange_registry_creation() {
    ...
    assert!(registry.exchange_count() >= 15, "Should have at least 15 exchanges");
}
...
fn test_global_market_data_aggregation() {
    ...
    market_data.update_price("AAPL", ExchangeId("NYSE".to_string()), Decimal::try_from(150.0).unwrap());
    market_data.update_price("AAPL", ExchangeId("NASDAQ".to_string()), Decimal::try_from(150_05).unwrap() / Decimal::from(100));
...
fn test_best_price_discovery() {
    ...
    market_data.update_price("BTCUSD", ExchangeId("BINANCE".to_string()), Decimal::try_from(50000.0).unwrap());
    market_data.update_price("BTCUSD", ExchangeId("COINBASE".to_string()), Decimal::try_from(50100.0).unwrap());
    market_data.update_price("BTCUSD", ExchangeId("KRAKEN".to_string()), Decimal::try_from(49950.0).unwrap());
...
fn test_cross_source_validation() {
    ...
    cross.add_price(DataSource::AlphaVantage, Decimal::try_from(150_10).unwrap() / Decimal::from(100));
    cross.add_price(DataSource::Finnhub, Decimal::try_from(149_90).unwrap() / Decimal::from(100));
...
fn test_outlier_detection() {
    ...
    cross.add_price(DataSource::YahooFinance, Decimal::try_from(150.0).unwrap());
    cross.add_price(DataSource::AlphaVantage, Decimal::try_from(150_20).unwrap() / Decimal::from(100));
    cross.add_price(DataSource::Finnhub, Decimal::try_from(149_80).unwrap() / Decimal::from(100));
...
fn test_data_source_quality() {
    ...
    let validation = aggregator.validate_against_paid(
        "AAPL",
        Decimal::try_from(150.02).unwrap(),
        "Bloomberg"
    );
...
fn test_free_vs_paid_comparison() {
    ...
    assert!(validation.deviation_pct < 0.1, "Free consensus should be close to paid");
}
