...
use investor_os::langgraph::{
    state::{SharedState, MarketRegime, TradingAction, ExecutionStatus, StateBuilder},
    nodes::{Node, NodeOutput, StartNode, CQCalculationNode},
...
async fn test_cq_calculation_node() {
    ...
    match result.unwrap() {
        NodeOutput::Continue(new_state) => {
            assert!(new_state.conviction_quotient.is_some());
            assert!((new_state.conviction_quotient.unwrap() - 0.7325).abs() < 0.001);
        }
        _ => panic!("Expected Continue"),
    }
}
...
async fn test_simple_graph_execution() {
    ...
    impl Node for TestTransformNode {
        ...
        async fn execute(&self, mut state: SharedState) -> Result<NodeOutput, NodeError> {
            state.metadata.insert("transformed".to_string(), serde_json::json!(true));
