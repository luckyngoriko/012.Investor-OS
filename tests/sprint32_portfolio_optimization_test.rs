...
use investor_os::portfolio_opt::{
    ...
    OptimizationConstraints, OptimizationObjective, OptimizedPortfolio, PortfolioOptimizationEngine,
    TradeAction,
};
...
fn test_golden_path_portfolio_optimization() {
    ...
    let mpt_portfolio = engine.optimize_mpt(
        &assets,
    ...
    assert_eq!(mpt_portfolio.objective, OptimizationObjective::MaximizeSharpe);
...
fn test_risk_parity_optimization() {
    ...
    let portfolio = engine.optimize_risk_parity(&assets, OptimizationConstraints::default()).unwrap();
    ...
    assert!(low_weight > high_weight, "Risk parity should overweight low-risk assets");
...
fn test_black_litterman_with_views() {
    ...
    let portfolio = engine.optimize_black_litterman(
        &assets,
        &market_caps,
        &views,
        OptimizationConstraints::default(),
...
fn test_portfolio_comparison() {
    ...
    let aggressive = engine.optimize_mpt(
        &assets,
        OptimizationObjective::MaximizeReturn,
        OptimizationConstraints::default(),
    ).unwrap();
    
    let conservative = engine.optimize_mpt(
        &assets,
...
fn test_portfolio_properties() {
    ...
    portfolio.weights.insert("AAPL".to_string(), Decimal::try_from(0.40).unwrap());
    portfolio.weights.insert("MSFT".to_string(), Decimal::try_from(0.30).unwrap());
    portfolio.weights.insert("GOOGL".to_string(), Decimal::try_from(0.30).unwrap());
...
fn test_multiple_objectives() {
    ...
    // Maximize return
    let max_return = engine.optimize_mpt(
        &assets,
        OptimizationObjective::MaximizeReturn,
        OptimizationConstraints::default(),
    ).unwrap();
    ...
    // Minimize risk
    let min_risk = engine.optimize_mpt(
        &assets,
        OptimizationObjective::MinimizeRisk,
        OptimizationConstraints::default(),
    ).unwrap();
    ...
    // Maximize Sharpe
    let max_sharpe = engine.optimize_mpt(
        &assets,
        OptimizationObjective::MaximizeSharpe,
        OptimizationConstraints::default(),
    ).unwrap();
