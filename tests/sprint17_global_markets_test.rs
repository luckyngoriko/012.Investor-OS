...
use investor_os::global::{
    GlobalMarketCoordinator, Region, ExchangeId,
...
async fn test_xetra_order() {
    ...
    coordinator.disconnect_all().await.expect("Should disconnect");
...
async fn test_hkex_connection() {
    ...
    // Свързване
    coordinator.connect_all().await.expect("Should connect to HKEX");
...
async fn test_fx_eurusd_order() {
    ...
    let result = treasury.convert(Currency::EUR, Currency::USD, Decimal::from(10000)).await;
    
    assert!(result.is_err());
    let err_msg = result.unwrap_err().to_string();
    assert!(
        err_msg.contains("Fiat not supported") || err_msg.contains("not supported"),
        "Expected fiat not supported error, got: {}", err_msg
    );
...
async fn test_24_7_coverage() {
    ...
    coordinator.connect_all().await.expect("Should connect to all");
...
async fn test_cross_exchange_arbitrage() {
    ...
    use investor_os::global::exchanges::{GenericExchange, ExchangeQuote};
    use chrono::Utc;
    ...
    assert!(spread > Decimal::ZERO, "Should detect arbitrage opportunity");
