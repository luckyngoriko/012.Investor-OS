...
use investor_os::ml::{
    Prediction,
    TrainingPipeline, TrainingConfig, CrossValidator,
    InferenceEngine, inference::InferenceConfig,
    FeatureEngine, FeatureConfig, PriceData,
...
fn test_model_ensemble_voting() {
    ...
    for (name, confidence, prediction) in &models {
        println!("   {}: confidence={:.2}, prediction={:.2}", name, confidence, prediction);
    ...
    let min_pred = models.iter().map(|(_, _, p)| *p).fold(f64::INFINITY, f64::min);
    let max_pred = models.iter().map(|(_, _, p)| *p).fold(f64::NEG_INFINITY, f64::max);
    
    assert!(ensemble_prediction >= min_pred && ensemble_prediction <= max_pred,
    ...
    println!("   Consensus: {} bullish, {} bearish", bullish_count, bearish_count);
...
fn test_realtime_inference_pipeline() {
    ...
    println!("   Confidence: {}%", prediction.confidence * Decimal::from(100));
    ...
    let signal = if prediction.value > threshold && prediction.confidence > high_confidence {
        ...
        "BUY"
    } else if prediction.value < Decimal::try_from(0.4).unwrap() && prediction.confidence > high_confidence {
        "STRONG_SELL"
    } else if prediction.value < Decimal::try_from(0.5).unwrap() {
        "SELL"
    } else {
...
fn test_model_drift_detection() {
    ...
    println!("   Drift threshold: {}%", config.drift_threshold * Decimal::from(100));
    ...
    println!("   Baseline accuracy: {}%", baseline_accuracy * Decimal::from(100));
    
    // Текуща точност (под прага)
    let current_accuracy = Decimal::try_from(0.71).unwrap();
    println!("   Current accuracy: {}%", current_accuracy * Decimal::from(100));
    ...
    assert!(drift_detected, "Should detect drift when accuracy drops below threshold");
...
fn test_feature_engineering_pipeline() {
    ...
    println!("   MACD fast/slow/signal: {}/{}/{}", config.macd_fast, config.macd_slow, config.macd_signal);
    ...
    let price_data: Vec<PriceData> = prices.iter().map(|&p| PriceData {
...
fn test_model_training_pipeline() {
    ...
    println!("   Validation accuracy: {:.1}%", validation_accuracy * 100.0);
...
fn test_batch_prediction_processing() {
    ...
    let inputs: Vec<Vec<f64>> = (0..batch_size)
        .map(|i| vec![i as f64 * 0.01, (i as f64 * 0.01).sin(), (i as f64 * 0.01).cos()])
        .collect();
    ...
    // Статистика
    let avg_confidence: Decimal = predictions.iter().map(|p| p.confidence).sum::<Decimal>() / Decimal::from(predictions.len() as i64);
    let avg_value: Decimal = predictions.iter().map(|p| p.value).sum::<Decimal>() / Decimal::from(predictions.len() as i64);
    
    println!("   Average confidence: {}%", avg_confidence * Decimal::from(100));
    println!("   Average prediction: {}", avg_value);
    
    // Distribution
    let bullish = predictions.iter().filter(|p| p.value > Decimal::try_from(0.5).unwrap()).count();
    let bearish = predictions.iter().filter(|p| p.value <= Decimal::try_from(0.5).unwrap()).count();
    
    println!("   Distribution: {} bullish, {} bearish", bullish, bearish);
    
    // Проверки
    assert_eq!(predictions.len(), batch_size, "Should generate one prediction per input");
    assert!(avg_confidence > Decimal::try_from(0.7).unwrap(), "Average confidence should be high");
...
fn test_live_signal_processing() {
    ...
    for (i, (price, _volume)) in ticks.iter().enumerate() {
        ...
        if let Some((direction, strength)) = signal {
            ...
            println!("   Tick {}: {} signal @ ${} (strength: {:.0}%)", i, direction, price, strength * 100.0);
        }
        
        last_price = *price;
    }
    
    println!("   Generated {} signals from {} ticks", signals.len(), ticks.len());
    
    // Quality scoring
    let avg_strength: f64 = signals.iter().map(|(_, _, s, _, _)| s).sum::<f64>() / signals.len() as f64;
...
fn test_model_evaluation_metrics() {
    ...
    println!("\n   Classification Report:");
    println!("   Positive class: P={:.1}%, R={:.1}%, F1={:.3}", precision * 100.0, recall * 100.0, f1);
...
fn test_end_to_end_ml_pipeline() {
    ...
    let raw_data: Vec<(&str, f64, f64, f64, f64, f64)> = (0..60)
        .map(|i| {
            ...
            ("2024-01-01", base, base + 5.0, base - 2.0, base + 3.0, 1000000.0 + (i as f64) * 10000.0)
        })
        .collect();
    ...
    let price_data: Vec<PriceData> = close_prices.iter().map(|&p| PriceData {
    ...
    println!("   Drift threshold: {}%", drift_config.drift_threshold * Decimal::from(100));
    println!("   Monitoring window: {} samples", drift_config.window_size);
    
    // Final validation
    println!("\n✅ Pipeline validation:");
    assert!(!features.is_empty(), "Should have features");
    assert!(prediction.confidence > Decimal::try_from(0.8).unwrap(), "Should have high confidence");
    ...
    println!("   - Output: Prediction={}, Confidence={}%", prediction.value, prediction.confidence * Decimal::from(100));
}
