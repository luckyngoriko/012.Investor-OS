...
use investor_os::ai_safety::{
    SafetyController, Action, ActionType,
    KillSwitch, KillSwitchConfig, KillSwitchState,
    CircuitBreaker, BreakerConfig, BreakerCondition, MarketState,
    TradingLimits, LimitEnforcer,
    OverrideType,
};
...
async fn test_kill_switch_manual_activation() {
    ...
    controller.activate_kill_switch("Emergency stop test".to_string(), "operator_1").await;
    ...
    // Reset kill switch
    controller.deactivate_kill_switch("admin").await.expect("Reset should succeed");
...
async fn test_kill_switch_no_trigger_on_normal_loss() {
    ...
    assert!(trigger.is_none(), "Kill switch should not trigger on 5% loss");
...
async fn test_circuit_breaker_daily_loss() {
    ...
    assert!(status.active_circuit_breakers.iter().any(|b| b.contains("Loss")));
...
async fn test_circuit_breaker_drawdown() {
    ...
    assert!(!events.is_empty(), "Drawdown circuit breaker should trigger");
    
    let status = controller.status().await;
    assert!(status.active_circuit_breakers.iter().any(|b| b.contains("Drawdown")));
}
...
async fn test_circuit_breaker_volatility_spike() {
    ...
    assert!(trigger.is_some(), "Volatility circuit breaker should trigger");
...
async fn test_trading_limits_daily_count() {
    ...
    match result {
        Err(e) => {
            let msg = e.to_string();
            assert!(msg.contains("limit") || msg.contains("Limit") || msg.contains("daily"), 
...
async fn test_ethical_constraints_prohibited_symbols() {
    ...
    assert!(violations.iter().any(|v| v.violation_type.contains("Prohibited")));
}
...
async fn test_pattern_detection_rapid_fire() {
    ...
    if !violations.is_empty() {
        assert!(violations.iter().any(|v| 
            v.violation_type.contains("Rapid Fire") || 
...
async fn test_audit_trail() {
    ...
    controller.activate_kill_switch("Audit test".to_string(), "test_operator").await;
...
async fn test_human_override_workflow() {
    ...
    // Request override
    let request = controller.request_override(
        OverrideType::ManualReview,
        "Test override".to_string(),
        action.clone(),
    ).await;
    
    assert_eq!(request.status, investor_os::ai_safety::override_ctrl::OverrideStatus::Pending);
    
    // Approve override
    controller.approve_override(request.id, "admin".to_string()).await.expect("Should approve");
