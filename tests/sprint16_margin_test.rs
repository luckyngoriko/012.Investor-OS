...
use investor_os::margin::{
    MarginManager, MarginAccount, Position, PositionSide,
...
fn test_leverage_position_open_close() {
    ...
    let btc_pnl = manager.close_position(account_id, "BTC", Decimal::from(55000)).unwrap();
    ...
    let eth_pnl = manager.close_position(account_id, "ETH", Decimal::from(2700)).unwrap();
    ...
    assert!(final_equity >= Decimal::from(100000), "Equity should not be less than initial");
    assert!(final_equity > equity_after_btc, "Equity should increase after closing ETH");
...
async fn test_liquidation_engine() {
    ...
    assert!(!liquidations.is_empty(), "Should have triggered liquidation");
...
fn test_cross_margin_collateral() {
    ...
    let btc_pnl = manager.close_position(account_id, "BTC", Decimal::from(52000)).unwrap();
    let eth_pnl = manager.close_position(account_id, "ETH", Decimal::from(2900)).unwrap();
    let sol_pnl = manager.close_position(account_id, "SOL", Decimal::from(110)).unwrap();
...
fn test_risk_metrics_calculation() {
    ...
    println!("   Distance to margin call: {:.2}%", metrics.distance_to_call);
...
async fn test_margin_call_scenario() {
    ...
    let pnl = manager.close_position(account_id, "BTC", Decimal::from(48000)).unwrap();
