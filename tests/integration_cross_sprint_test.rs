...
fn test_ml_strategy_portfolio_integration() {
    ...
    assert!(
        !optimized.is_empty(),
        "Portfolio optimization should produce allocations"
    );
}
...
fn test_agents_analytics_monitoring_integration() {
    use investor_os::agent::{AgentConfig, MultiAgentSystem};
...
fn test_security_audit_compliance_integration() {
    ...
    use investor_os::security::{AuditEvent, AuditLogger, SecurityPolicy};
...
fn test_production_readiness_integration() {
    ...
    assert!(
        ...
        "Deployment config should be valid"
    );
}
