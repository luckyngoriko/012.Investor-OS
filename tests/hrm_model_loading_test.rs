...
use investor_os::hrm::{HRM, HRMConfig, HRMBuilder};
use std::path::Path;
...
fn test_load_safetensors_model() {
    ...
    let hrm = hrm.unwrap();
    assert!(hrm.is_ready(), "Model should be ready after loading weights");
...
fn test_golden_dataset() {
    ...
    // Load golden cases
    let golden_data: serde_json::Value = serde_json::from_str(
        &fs::read_to_string(golden_path).expect("Should read golden file")
    ...
    for case in cases.iter().take(10) {  // Test first 10
        ...
        if diff <= tolerance {
            println!("  ✅ {}: expected {:.2}, got {:.2} (diff: {:.2})", 
...
fn test_batch_performance() {
    ...
    assert!(avg_ms < 10.0, "Average inference too slow: {:.3} ms", avg_ms);
}
