...
use investor_os::treasury::{
    security::{SecurityCheck, SecurityManager, TfaMethod},
    Currency, TransactionStatus, Treasury, WithdrawalDestination,
};
...
async fn test_withdrawal_security_check() {
    ...
    assert!(
        withdrawal_small.is_ok(),
        "Small withdrawal should succeed without 2FA"
    );
    ...
    assert!(
        withdrawal_large.is_ok(),
        "Large withdrawal should succeed with valid 2FA"
    );
    ...
    assert!(
        withdrawal_invalid_2fa.is_err(),
        "Withdrawal with invalid 2FA should fail"
    );
    ...
    assert!(
        withdrawal_over_limit.is_err(),
        "Withdrawal over daily limit should fail"
    );
...
async fn test_cold_wallet_integration() {
    ...
    assert!(
        cold_address.starts_with("bc1")
            || cold_address.starts_with("1")
            || cold_address.starts_with("3"),
    ...
    assert!(
        cold_balance >= Decimal::from(5),
        "Cold balance should be at least 5 BTC"
    );
    ...
    assert!(
        cold_info.total_btc > Decimal::ZERO,
        "Should have BTC in cold storage"
    );
    assert!(
        cold_info.multi_sig_threshold > 0,
        "Should require multi-sig"
    );
    assert!(
        !cold_info.signers.is_empty(),
        "Should have signers configured"
    );
    ...
    println!(
        "   - Multi-sig: {}/{}",
        cold_info.multi_sig_threshold,
        cold_info.signers.len()
    );
}
...
async fn test_performance_1000_concurrent_users() {
    ...
    assert_eq!(
        completed_deposits, CONCURRENT_USERS,
        "All deposits should complete"
    );

    let deposit_duration = start.elapsed();
    println!(
        "   - {} deposits completed in {:?}",
        completed_deposits, deposit_duration
    );
    ...
    println!(
        "   - {} balance checks completed in {:?}",
        completed_balances, balance_duration
    );
    ...
    println!(
        ...
        completed_conversions, fx_duration
    );
    ...
    println!(
        "   - Ops/sec: {:.0}",
        (CONCURRENT_USERS * 3) as f64 / total_duration.as_secs_f64()
    );

    // Проверка на performance threshold
    assert!(
        total_duration < MAX_DURATION * 3, // Позволяваме 3x за всички операции
