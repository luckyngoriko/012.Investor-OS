...
"#;
    ...
    assert!(
        sections.contains(&"Business".to_string()),
        "Should have Business section"
    );
    assert!(
        sections.contains(&"Risk Factors".to_string()),
        "Should have Risk Factors section"
    );
    assert!(
        sections.contains(&"MD&A".to_string()),
        "Should have MD&A section"
    );
}
...
"#;
    ...
    let has_revenue = all_metrics.iter().any(|m| m.metric_type == "revenue");
...
"#;
    ...
    let forward_looking_chunks: Vec<_> = chunks
        .iter()
        .filter(|c| c.metadata.contains_forward_looking)
        .collect();

    assert!(
        !forward_looking_chunks.is_empty(),
