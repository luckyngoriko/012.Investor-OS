...
fn test_compression_policy_exists() {
    ...
    assert!(
        migration.contains("timescaledb.compress"),
        "Migration should enable TimescaleDB compression"
    );
    assert!(
        migration.contains("add_compression_policy"),
        "Migration should add compression policies"
    );
    assert!(
        migration.contains("INTERVAL '7 days'"),
...
fn test_compression_expected_ratio() {
    ...
    assert!(
        expected_signals_ratio > 0.85,
        "Signals should compress >85%"
    );
}
