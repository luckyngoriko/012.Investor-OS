...
Tim Cook - CEO: We expect continued strong performance."#;
    ...
    let chunks = parser.parse(transcript, "AAPL", Utc::now())
        .await
...
Tim Cook - CEO: Our margins expanded significantly."#;
    ...
    assert!(sections.contains(&"Prepared Remarks".to_string()), 
        "Should have Prepared Remarks section");
    assert!(sections.contains(&"Q&A".to_string()), 
...
This guidance reflects our current view."#;
    ...
    let forward_looking = chunks.iter()
        .any(|c| c.metadata.contains_forward_looking);
