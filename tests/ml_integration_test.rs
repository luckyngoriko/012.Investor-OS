...
use investor_os::ml::{
    inference::InferenceConfig, FeatureConfig, FeatureEngine, MlPortfolioOptimizer,
    MlPortfolioViews, MlPositionSizer, MlSignalGenerator, MlTradingDecision, MlTradingPipeline,
...
fn test_position_sizing_with_confidence() {
    ...
    for (confidence, desc) in test_cases {
        let size = sizer.calculate_size_with_confidence(capital, entry, stop_loss, confidence);
...
fn test_ml_portfolio_views() {
    ...
    println!(
        "   High confidence views (>=70%): {}",
        high_confidence.predictions.len()
    );
...
fn test_confidence_position_size_comparison() {
    ...
    let high_conf_size = sizer
        .calculate_size_with_confidence(capital, entry, stop, Decimal::try_from(0.95).unwrap())
        .unwrap_or(Decimal::ZERO);

    let low_conf_size = sizer
        .calculate_size_with_confidence(capital, entry, stop, Decimal::try_from(0.55).unwrap())
        .unwrap_or(Decimal::ZERO);
...
fn test_complete_ml_trading_workflow() {
    ...
    for symbol in &symbols {
        ...
        if let Some(decision) =
            pipeline.process_market_data(symbol, &price_history, capital_per_trade)
        {
            println!(
                "   {}: {:?} - {} shares",
...
fn test_ml_integration_with_risk() {
    ...
    for symbol in &symbols {
        ...
        if let Some(decision) =
            pipeline.process_market_data(symbol, &price_history, capital / Decimal::from(5))
        {
