...
impl DeploymentValidator {
    ...
    pub fn calculate_score(&self, health: &HealthResponse) -> f32 {
        ...
        let passed = health.checks.values()
...
impl K8sResourceValidator {
    ...
    pub fn validate_resource_limits(&self, requests: &ResourceSpec, limits: &ResourceSpec) -> bool {
        ...
        let memory_valid = Self::parse_memory(&limits.memory) >= Self::parse_memory(&requests.memory);
...
mod tests {
    ...
    fn test_validate_health() {
        ...
        checks.insert("api".to_string(), CheckStatus {
            status: "pass".to_string(),
            message: None,
            response_time_ms: 20,
        });
    ...
    fn test_calculate_health_score() {
        ...
        let mut checks = HashMap::new();
        checks.insert("check1".to_string(), CheckStatus {
            status: "pass".to_string(),
            message: None,
            response_time_ms: 10,
        });
        checks.insert("check2".to_string(), CheckStatus {
            status: "pass".to_string(),
            message: None,
            response_time_ms: 10,
        });
        checks.insert("check3".to_string(), CheckStatus {
            status: "fail".to_string(),
            message: None,
            response_time_ms: 10,
        });
    ...
    fn test_parse_memory() {
        assert_eq!(K8sResourceValidator::parse_memory("512Mi"), 512 * 1024 * 1024);
        assert_eq!(K8sResourceValidator::parse_memory("1Gi"), 1024 * 1024 * 1024);
