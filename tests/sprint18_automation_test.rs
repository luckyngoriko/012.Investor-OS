...
use investor_os::execution::{
    ExecutionEngine, Order, OrderSide, OrderType, TimeInForce, Fill,
    algorithms::{TWAPExecutor, VWAPExecutor, IcebergExecutor, AlgorithmSelector},
    router::{SmartRouter, RouteDecision},
    venue::{Venue, VenueQuote, VenueAnalyzer, FeeStructure},
...
async fn test_twap_execution() {
    ...
    println!("✅ Created TWAP order: {} BTC over {}s in {} slices", 
        order.quantity, 60, 5);
    ...
    }).await.expect("TWAP should execute");
    ...
    assert_eq!(total_filled, Decimal::from(100), "Total should equal order quantity");
    
    // Всяка fill трябва да е приблизително 20 BTC (100/5)
    for (i, fill) in fills.iter().enumerate() {
        println!("   Slice {}: {} BTC @ ${}", i + 1, fill.quantity, fill.price);
...
async fn test_vwap_algorithm() {
    ...
    assert!(open_weight > midday_weight, "Open should have higher volume than midday");
    assert!(close_weight > midday_weight, "Close should have higher volume than midday");
    
    // VWAP поръчка (1 сек за тест)
    let order = Order::vwap("ETH", OrderSide::Sell, Decimal::from(240), 1);
    let vwap = VWAPExecutor::new();
    
    let fills = vwap.execute(&order, &profile, |qty| {
        async move {
            Ok(Fill {
                id: uuid::Uuid::new_v4(),
                order_id: order.id,
    ...
    }).await.expect("VWAP should execute");
...
fn test_smart_routing() {
    ...
    let best_buy = router.venue_analyzer().get_best_quote("BTC", OrderSide::Buy);
    assert!(best_buy.is_some());
    
    let best = best_buy.unwrap();
    println!("   Best ask: ${} from {} (latency: {}ms)", 
        best.ask, best.venue.name(), best.latency_ms);
    ...
    println!("   Route decision: Execute at {}", decision.primary_venue.name());
    println!("   Expected cost: ${}", decision.expected_cost);
    
    // Проверка на venue scoring
    let scores = router.venue_analyzer().score_venues("BTC", Decimal::from(10), OrderSide::Buy);
    println!("\n   Venue scores:");
    for score in &scores {
        println!("     {}: composite={}", score.venue.name(), score.composite_score);
    }
...
async fn test_iceberg_execution() {
    ...
    println!("✅ Created iceberg order: {} BTC (displayed: {})", 
    ...
    // Изпълнение
    let fills = iceberg.execute(&order, |_child| {
        async move {
            Ok(Fill {
                id: uuid::Uuid::new_v4(),
                order_id: _child.id,
    ...
    }).await.expect("Iceberg should execute");
...
fn test_execution_cost_calculation() {
    ...
    println!("   Small order (1 BTC) cost: {} bps", small_cost.total_cost_bps);
    ...
    println!("   Large order (100 BTC) cost: {} bps", large_cost.total_cost_bps);
    ...
    println!("     Total: ${} ({} bps)", large_cost.total_cost, large_cost.total_cost_bps);
...
fn test_algorithm_selection() {
    ...
    println!("✅ Medium order (50 BTC): {} - {}", algo_medium, reason_medium);
    ...
    println!("✅ Large order (200 BTC): {} - {}", algo_large, reason_large);
    ...
    println!("✅ Very large order (2000 BTC): {} - {}", algo_xl, reason_xl);
...
async fn test_full_automation_lifecycle() {
    ...
    println!("   Best ask: ${} at {}", best.unwrap().ask, best.unwrap().venue.name());
    ...
    let fills = engine.submit_order(&twap_order).await
...
fn test_multi_venue_execution() {
    ...
    let venues = vec![
        (Venue::Binance, Decimal::from(1000), Decimal::try_from(0.001).unwrap()),
        (Venue::Coinbase, Decimal::from(800), Decimal::try_from(0.0015).unwrap()),
        (Venue::Kraken, Decimal::from(500), Decimal::try_from(0.002).unwrap()),
    ];
    ...
    for (venue, liquidity, fee) in &venues {
        println!("   {}: liquidity={}, fee={}%", venue.name(), liquidity, fee * Decimal::from(100));
    }
    ...
    if order.quantity > single_venue_liquidity {
        println!("   Order size ({}) exceeds single venue liquidity ({})", 
    ...
    for (i, d) in decisions.iter().enumerate() {
        println!("     Venue {}: {} - {}", i + 1, d.primary_venue.name(), d.reason);
    }
...
fn test_fee_structure_comparison() {
    ...
    println!("   Binance taker: ${} ({} fee)", cost_binance, fee_binance.taker_fee);
    println!("   Coinbase taker: ${} ({} fee)", cost_coinbase, fee_coinbase.taker_fee);
    
    assert!(cost_binance < cost_coinbase);
    println!("   ✓ Binance is cheaper by ${}", cost_coinbase - cost_binance);
