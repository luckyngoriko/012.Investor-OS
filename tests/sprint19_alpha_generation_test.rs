...
use investor_os::strategy_selector::{
    StrategySelector, SelectionCriteria, StrategyType, MarketRegime,
    StrategyRecommender, Recommendation,
    StrategySwitcher, SwitchConfig,
};
...
fn test_signal_generation_pipeline() {
    ...
    assert!(signals.insider_flow_ratio > 1.0, "Positive insider flow expected");
    assert!(signals.insider_cluster_signal, "Cluster signal should be active");
    
    // Проверка на sentiment
    assert!(signals.news_sentiment > 0.0, "Positive news sentiment expected");
    assert!(signals.social_sentiment > 0.0, "Positive social sentiment expected");
    
    // Проверка на technical индикатори
    assert!(signals.rsi_14 > 30.0 && signals.rsi_14 < 70.0, "RSI should be in neutral range");
...
fn test_composite_quality_calculation() {
    ...
    println!("✅ Generated feature vector with {} features", features.features.len());
...
fn test_backtest_walk_forward() {
    ...
    println!("   Commission: {}%", config.commission_rate * Decimal::from(100));
    ...
    println!("   Annualized Return: {:.2}%", mock_result.annualized_return);
    ...
    // Проверки
    assert!(mock_result.sharpe_ratio > Decimal::ZERO, "Sharpe should be positive");
    assert!(mock_result.max_drawdown <= Decimal::ZERO, "Max DD should be negative or zero");
    ...
    let calculated_win_rate = if mock_result.total_trades > 0 {
        Decimal::from(mock_result.winning_trades as i64) / Decimal::from(mock_result.total_trades as i64) * Decimal::from(100)
    } else {
        Decimal::ZERO
    };
    let diff = (calculated_win_rate - mock_result.win_rate).abs();
    println!("   Calculated Win Rate: {}%, Stored: {}%, Diff: {}%", calculated_win_rate, mock_result.win_rate, diff);
    assert!(diff < Decimal::from(5), "Win rate calculation mismatch should be within 5%");
...
fn test_risk_metrics_calculation() {
    ...
    println!("   Max Drawdown: {:.2}%", analyzer.max_drawdown() * Decimal::from(100));
    println!("   VaR (95%): {:.2}%", analyzer.var(Decimal::try_from(0.95).unwrap()) * Decimal::from(100));
    
    assert!(analyzer.sharpe_ratio() > Decimal::ZERO, "Sharpe should be positive for these returns");
    assert!(analyzer.max_drawdown() <= Decimal::ZERO, "Max DD should be negative or zero");
...
fn test_strategy_regime_selection() {
    ...
    let regimes = vec![
        (MarketRegime::Trending, vec![StrategyType::Momentum, StrategyType::TrendFollowing]),
        (MarketRegime::Ranging, vec![StrategyType::MeanReversion, StrategyType::PairsTrading]),
        (MarketRegime::VolatilityExpansion, vec![StrategyType::Breakout]),
        (MarketRegime::LowVolatility, vec![StrategyType::MarketMaking]),
    ];
    ...
    for (regime, expected_strategies) in regimes {
        ...
        for strategy in &expected_strategies {
            ...
            println!("   - {:?}: {}", strategy, if is_suitable { "✓ Suitable" } else { "✗ Not suitable" });
            assert!(is_suitable, "{:?} should be suitable for {:?}", strategy, regime);
        }
    }
...
fn test_ml_feature_engineering() {
    ...
    let quality_idx = features.feature_names.iter().position(|n| n == "quality_score").unwrap();
    let value_idx = features.feature_names.iter().position(|n| n == "value_score").unwrap();
    let interaction_idx = features.feature_names.iter().position(|n| n == "quality_x_value").unwrap();
    ...
    assert!((interaction - expected_interaction).abs() < 0.001, 
        "Quality x Value interaction mismatch: {} vs {}", interaction, expected_interaction);
    ...
    let price_history = vec![100.0, 102.0, 101.0, 105.0, 108.0, 106.0, 110.0, 112.0, 111.0, 115.0];
...
fn test_strategy_recommendation() {
    ...
    println!("   Strategy: {:?} - {}", recommendation.strategy_type, recommendation.strategy_name);
    println!("   Rank: {}", recommendation.rank);
    println!("   Score: {:.0}%", recommendation.score * 100.0);
    println!("   Expected Return: {:.1}%", recommendation.expected_return * 100.0);
...
fn test_strategy_switching() {
    ...
    println!("   Min confidence: {:.0}%", config.confidence_threshold * 100.0);
    println!("   Min improvement: {:.0}%", config.min_score_improvement * 100.0);
...
fn test_full_alpha_generation_pipeline() {
    ...
    let momentum_suitable = StrategyType::Momentum.suitable_regimes().contains(&trending_regime);
    let mr_suitable = StrategyType::MeanReversion.suitable_regimes().contains(&trending_regime);
    
    println!("   Regime: Trending");
    println!("   Momentum suitable: {}", momentum_suitable);
    println!("   Mean Reversion suitable: {}", mr_suitable);
    println!("   Criteria: Sharpe > {:.1}, Max DD < {:.0}%", criteria.min_sharpe, criteria.max_drawdown * 100.0);
    
    assert!(momentum_suitable, "Momentum should be suitable for trending");
    assert!(!mr_suitable, "Mean Reversion should NOT be suitable for trending");
    ...
    assert!(sharpe > Decimal::try_from(1.5).unwrap(), "Sharpe should exceed 1.5 threshold");
...
fn test_performance_attribution() {
    ...
    for (factor, ret) in &factor_returns {
        ...
        println!("   {}: {:.2}% (contribution: {:.1}%)", factor, ret * Decimal::from(100), contribution);
    }
    println!("   Total Return: {:.2}%", total_return * Decimal::from(100));
    
    // Проверка
    assert!(total_return > Decimal::ZERO, "Total return should be positive");
    assert!(factor_returns[2].1 > factor_returns[1].1, "Momentum should outperform Value");
