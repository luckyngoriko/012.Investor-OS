use super::{PrecisionSettings, Recorder, RecorderError};
...
/// File recorder using compact binary serialization.
#[derive(new, Debug, Default, Clone)]
pub struct BinFileRecorder<S: PrecisionSettings> {
    _settings: PhantomData<S>,
}

/// File recorder using compact binary serialization compressed with gzip.
#[derive(new, Debug, Default, Clone)]
...
impl<S: PrecisionSettings, B: Backend> Recorder<B> for BinGzFileRecorder<S> {
    ...
    fn load_item<I: DeserializeOwned>(&self, mut file: Self::LoadArgs) -> Result<I, RecorderError> {
        ...
        let state = rmp_serde::decode::from_read(&mut reader)
            .map_err(|err| RecorderError::Unknown(err.to_string()))?;
