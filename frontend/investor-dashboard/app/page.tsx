...
import { showTradeNotification } from "@/components/notification-center";
import {
  TrendingUp,
  TrendingDown,
  Activity,
...
const tradeProposals = [
  ...
  },
  {
    ...
    factors: { pegy: 0.72, insider: 0.65, sentiment: 0.8, technical: 0.91 },
    regime: "Uncertain",
  },
];
...
const StatCard = ({
  ...
  >
    <div className="flex items-start justify-between mb-4">
      <div
        className={`p-3 rounded-xl ${changeType === "positive" ? "bg-emerald-500/10" : changeType === "negative" ? "bg-rose-500/10" : "bg-blue-500/10"}`}
      >
        <Icon
          className={`w-6 h-6 ${changeType === "positive" ? "text-emerald-400" : changeType === "negative" ? "text-rose-400" : "text-blue-400"}`}
        />
      </div>
      {change && (
        <div
          className={`flex items-center gap-1 text-sm font-medium ${changeType === "positive" ? "text-emerald-400" : "text-rose-400"}`}
        >
          {changeType === "positive" ? (
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span
              className={`text-xs font-medium px-2 py-0.5 rounded-full ${isBuy ? "bg-emerald-500/20 text-emerald-400" : "bg-rose-500/20 text-rose-400"}`}
            >
              {proposal.action}
            </span>
          </div>
          <p className="text-sm text-gray-400">
            {proposal.qty} shares @ ${proposal.price.toFixed(2)}
          </p>
        </div>
      ...
      <div className="flex gap-2 mb-3">
        {Object.entries(proposal.factors).map(([key, value]) => (
          <div
            key={key}
            className="flex-1 text-center p-2 rounded-lg bg-gray-900/50"
          >
            <p className="text-xs text-gray-500 uppercase">{key}</p>
            <p
              className={`text-sm font-medium ${(value as number) >= 0.8 ? "text-emerald-400" : (value as number) >= 0.6 ? "text-amber-400" : "text-gray-400"}`}
            >
...
export default function DashboardPage() {
  ...
  const handleConfirmProposal = (index: number) => {
    ...
    showTradeNotification(
      proposal.symbol,
      proposal.action.toLowerCase() as "buy" | "sell",
      proposal.qty,
      proposal.price,
    );
  ...
  const totalValue = positions.reduce(
    (sum, pos) => sum + pos.qty * pos.currentPrice,
    0,
  );
              ...
              <div className="flex items-center gap-6">
                <div className="text-center">
                  ...
                  <p className="text-lg font-semibold text-white">
                    {marketRegime.vix.toFixed(1)}
                  </p>
                </div>
                <div className="text-center">
                  <p className="text-xs text-gray-500 uppercase">Breadth</p>
                  <p className="text-lg font-semibold text-white">
                    {Math.round(marketRegime.breadth * 100)}%
                  </p>
                </div>
                <div className="text-center">
                  ...
                  <tbody>
                    {positions.map((pos, index) => {
                      ...
                      const pnlPercent =
                        ...
                        100;
                      const weight =
                        ((pos.qty * pos.currentPrice) / totalValue) * 100;
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            <div
                              className={`flex items-center gap-1 ${pnl >= 0 ? "text-emerald-400" : "text-rose-400"}`}
                            >
                              {pnl >= 0 ? (
                                <TrendingUp className="w-4 h-4" />
                              ) : (
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      New to Investor OS? Take a tour or read the docs
                    </p>
                  </div>
                </div>
                <div className="flex gap-3">
