...
use super::{edges::*, nodes::*, state::SharedState, GraphError};
...
impl ExecutableGraph {
    ...
    pub async fn execute(&self, mut state: SharedState) -> Result<SharedState, GraphError> {
        ...
        let mut current = *self
            .node_indices
            .get(start_node)
            .ok_or(GraphError::ExecutionError(format!(
        ...
        loop {
                ...
                Ok(NodeOutput::Jump(target, new_state)) => {
                    ...
                    current =
                        *self
                            .node_indices
                            .get(&target)
                            .ok_or(GraphError::NodeError(format!(
            ...
            if neighbors.is_empty() {
                ...
                return Err(GraphError::ExecutionError(format!(
                    "No valid transition from '{}'",
                    node_info.name
                )));
            }

            if neighbors.len() > 1 {
                // Multiple outgoing edges: petgraph returns edges in insertion order,
                // which is deterministic for a given graph construction sequence.
                // We always take the first neighbor, which is the earliest-added edge.
                tracing::warn!(
                    node = %node_info.name,
                    paths = neighbors.len(),
                    "Multiple outgoing edges detected; taking first (insertion-order) path"
                );
            }
...
impl GraphBuilder {
    ...
    {
        self.graph
            .add_conditional_edge(&from.into(), condition, &to.into());
        self
    }
