...
pub use harvesting::{HarvestOpportunity, HarvestResult, TaxLossHarvester};
pub use reporting::{TaxForm, TaxReport, TaxReportingEngine, TaxYear};
...
impl TaxLot {
    ...
    pub fn unrealized_pnl(&self) -> Option<Decimal> {
        self.current_price
            .map(|price| (price - self.purchase_price) * self.quantity - self.fees)
    }
...
impl TaxEngine {
    ...
    pub fn find_harvest_opportunities(&self) -> Vec<HarvestOpportunity> {
        ...
        self.harvester
            .find_opportunities(self.lots.values().collect())
    }
    ...
    pub fn execute_harvest(&mut self, lot_id: Uuid) -> Result<HarvestResult> {
        ...
        if let Some(violation) = self.wash_sale_monitor.check_wash_sale(&lot) {
            return Err(TaxError::WashSaleViolation(format!(
    ...
    pub fn get_realized_for_year(&self, year: i32) -> RealizedSummary {
        let year_gains: Vec<_> = self
            .realized_gains
            .iter()
            .filter(|g| g.sale_date.year() == year)
            .collect();

        let short_term: Decimal = year_gains
            .iter()
            .filter(|g| g.is_short_term)
            .map(|g| g.gain_loss)
            .sum();

        let long_term: Decimal = year_gains
            .iter()
            .filter(|g| !g.is_short_term)
            .map(|g| g.gain_loss)
            .sum();
        ...
        RealizedSummary {
            ...
            wash_sale_adjustments: year_gains.iter().map(|g| g.adjustment_amount).sum(),
        }
    }
