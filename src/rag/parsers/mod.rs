...
fn extract_financial_metrics(text: &str) -> Vec<FinancialMetric> {
    ...
    let patterns = [
        ("revenue", r"(?:revenue|sales)\s*(?:of\s*)?\$?([\d,]+\.?\d*)\s*(million|billion|M|B)?"),
        ("eps", r"(?:earnings per share|EPS)\s*(?:of\s*)?\$?([\d,]+\.?\d*)"),
        ("net income", r"(?:net income|profit)\s*(?:of\s*)?\$?([\d,]+\.?\d*)\s*(million|billion|M|B)?"),
        ("guidance", r"(?:guidance|outlook)\s*(?:for\s*)?(?:FY\s*)?(\d{4})?\s*(?:is\s*)?\$?([\d,]+\.?\d*)"),
    ];
