...
impl EarningsParser {
    ...
    pub async fn parse(
        ...
        let prepared_remarks: Vec<_> = segments.iter()
            .filter(|s| matches!(s.speaker_role, SpeakerRole::Ceo | SpeakerRole::Cfo | SpeakerRole::Cio | SpeakerRole::Executive))
            .map(|s| format!("{} ({}): {}", s.speaker, self.role_to_string(&s.speaker_role), s.content))
            .collect();
        
        let qa_exchange: Vec<_> = segments.iter()
        ...
        if !prepared_remarks.is_empty() {
            ...
            let prepared_chunks = chunk_text(&prepared_content, super::CHUNK_SIZE, super::CHUNK_OVERLAP);
...
Tim Cook - CEO: Our margins improved significantly."#;
        ...
        assert!(segments.iter().any(|s| matches!(s.speaker_role, SpeakerRole::Ceo)));
    }
...
Luca Maestri - CFO: Our EPS was $2.50."#;
        ...
        assert!(chunks[0].metadata.financial_metrics.iter().any(|m| m.metric_type == "revenue"));
    }
}
