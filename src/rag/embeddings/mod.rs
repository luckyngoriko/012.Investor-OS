...
impl EmbeddingGenerator {
    ...
    pub async fn generate(&self, text: &str) -> Result<Vec<f32>> {
        match &self.model {
            #[cfg(feature = "local-embeddings")]
            EmbeddingModel::Local(model) => self.generate_local(text, model).await,
    ...
    async fn generate_local(&self, text: &str, model: &LocalModel) -> Result<Vec<f32>> {
        ...
        // Run inference
        let outputs: DynOrtTensor<f32, _> = model
            .session
            .run(inputs)
            .map_err(|e| RagError::Embedding(e.to_string()))?
            .extract_tensor(0)
    ...
    async fn generate_openai(&self, text: &str, api_key: &str, model: &str) -> Result<Vec<f32>> {
        ...
        if !response.status().is_success() {
            ...
            return Err(RagError::ExternalApi(format!(
                "OpenAI API error: {}",
                error_text
            )));
        }
