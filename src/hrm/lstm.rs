...
impl<B: Backend> LoadableLinear<B> {
    ...
    pub fn from_tensors(weight: Tensor<B, 2>, bias: Option<Tensor<B, 1>>) -> Self {
        Self {
            weight: Param::from_tensor(weight),
    ...
    pub fn forward(&self, input: Tensor<B, 2>) -> Tensor<B, 2> {
        ...
        match &self.bias {
            Some(bias) => {
                ...
                let bias_expanded: Tensor<B, 2> =
                    bias.val().reshape([1, bias.val().shape().dims[0]]);
