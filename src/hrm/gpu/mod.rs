...
pub use detector::{GpuDetector, DetectedBackend};
pub use backend::{GpuBackend, BackendType, DeviceInfo, CpuBackend};
...
fn create_backend(backend_type: BackendType) -> Result<Box<dyn GpuBackend>, String> {
    match backend_type {
        ...
        #[cfg(feature = "rocm")]
        BackendType::Rocm => {
            backend::RocmBackend::new()
                .map(|b| Box::new(b) as Box<dyn GpuBackend>)
        }
        
        #[cfg(feature = "intel")]
        BackendType::OneApi => {
            backend::OneApiBackend::new()
                .map(|b| Box::new(b) as Box<dyn GpuBackend>)
        }
        
        BackendType::Cpu => {
            Ok(Box::new(backend::CpuBackend::new()) as Box<dyn GpuBackend>)
        }
