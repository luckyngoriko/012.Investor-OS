...
use super::{
    memory::{ExitReason, TradeOutcome},
    Action, DecimalExt, EpochMetrics, TradingDecision,
};
...
impl Default for SimulatorConfig {
    fn default() -> Self {
        Self {
            ...
            max_position_pct: Decimal::from_str_exact("0.20")
                .unwrap_or(Decimal::from(20) / Decimal::from(100)),
            max_positions: 5,
            default_stop_loss_pct: Decimal::from_str_exact("0.05")
                .unwrap_or(Decimal::from(5) / Decimal::from(100)),
            default_take_profit_pct: Decimal::from_str_exact("0.10")
                .unwrap_or(Decimal::from(10) / Decimal::from(100)),
        }
    }
}
...
impl PaperTradingSimulator {
    ...
    fn execute_buy(
        ...
        let position = self
            ...
            .or_insert(Position {
                ...
                stop_loss: decision
                    .quantity
                    .map(|_| price * (Decimal::ONE - Decimal::from_str_exact("0.05").unwrap())),
                take_profit: decision
                    .quantity
                    .map(|_| price * (Decimal::ONE + Decimal::from_str_exact("0.10").unwrap())),
            });

        // Update position
        let total_quantity = position.quantity + quantity;
        position.avg_entry_price = (position.avg_entry_price * Decimal::from(position.quantity)
    ...
    pub fn calculate_metrics(&self) -> EpochMetrics {
        ...
        let realized_trades: Vec<_> = self
            .portfolio
            .trade_history
            .iter()
