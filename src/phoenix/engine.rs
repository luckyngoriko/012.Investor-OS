...
impl DecisionMaker for RuleBasedAdapter {
    ...
    fn generate_lesson(&self, experience: &TradingExperience) -> String {
        if experience.outcome.success {
            format!("Success: Captured {:.1}% profit", experience.outcome.profit_loss_pct)
        } else {
...
impl PhoenixEngine {
    ...
    pub fn with_llm_strategist(config: PhoenixConfig) -> Self {
        ...
        let llm_strategist = LlmStrategist::new(
            provider,
            super::strategist::StrategistConfig::default(),
        );
        ...
        Self {
            ...
            simulator: PaperTradingSimulator::new(sim_config),
            assessor: GraduationAssessor::new(config.graduation_config.clone()),
            strategist: Box::new(LlmStrategistAdapter { strategist: llm_strategist }),
            current_epoch: 0,
        }
    }
    ...
    pub fn run_training_epoch(&mut self, market_data: Vec<MarketDataPoint>) -> TrainingResult {
        ...
        while let Some(data) = self.simulator.current_data() {
            ...
            let current_pos = self.simulator.portfolio().positions.get(&ticker).map(|p| p.quantity);
    ...
    /// Run multiple epochs
    pub fn train(&mut self, epochs: u32, market_data_fn: impl Fn(u32) -> Vec<MarketDataPoint>) -> Vec<TrainingResult> {
    ...
    fn determine_trend(&self, data: &MarketDataPoint) -> TrendDirection {
        match (data.indicators.sma_20, data.indicators.sma_50, data.close) {
            (Some(sma20), Some(sma50), close) => {
                let price = close;
                if price > sma20 && sma20 > sma50 * rust_decimal::Decimal::from(102) / rust_decimal::Decimal::from(100) {
                    TrendDirection::StrongUptrend
                } else if price > sma20 {
                    TrendDirection::Uptrend
                } else if price < sma20 && sma20 < sma50 * rust_decimal::Decimal::from(98) / rust_decimal::Decimal::from(100) {
    ...
    fn classify_regime(&self, data: &MarketDataPoint) -> MarketRegime {
        ...
        let volatility = data.indicators.atr.map(|atr| {
