...
impl MLApiOrchestrator {
    ...
    pub async fn generate(&self, prompt: &str) -> Result<String, LLMError> {
        ...
        for provider in &self.providers {
            ...
            if self
                .cost_tracker
                .is_rate_limited(provider.name().to_lowercase().as_str())
            {
            ...
            match provider.generate(prompt).await {
                Ok(response) => {
                    ...
                    self.cost_tracker
                        .record_usage(&provider_name, input_tokens, output_tokens);
                ...
                Err(e) => {
                    ...
                    if matches!(e, LLMError::RateLimitExceeded) {
                        self.cost_tracker
                            .update_rate_limit(&provider.name().to_lowercase(), 0, 60);
                    }
                }
            }
        }
...
pub fn estimate_tokens(input: &str, output: &str) -> (u64, u64) {
