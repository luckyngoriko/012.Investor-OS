...
impl YieldOptimizer {
    ...
    /// Allocate funds to a yield protocol using explicit protocol selection.
    pub async fn allocate(
        ...
        if amount <= Decimal::ZERO {
            return Err(TreasuryError::InvalidAmount(
                "yield allocation amount must be positive".to_string(),
            ));
        }
        ...
        let selected_protocol = self
            .protocols
            .iter()
            .find(|p| p.name.eq_ignore_ascii_case(&protocol))
            .ok_or_else(|| {
                TreasuryError::UnsupportedCurrency(format!("unknown yield protocol {}", protocol))
            })?;

        if !selected_protocol.supported_currencies.contains(&currency) {
            return Err(TreasuryError::UnsupportedCurrency(format!(
        ...
        let position = YieldPosition {
            id: Uuid::new_v4(),
            protocol: selected_protocol.name.clone(),
            currency,
            amount,
            apy: selected_protocol.base_apy + selected_protocol.reward_apy.unwrap_or(Decimal::ZERO),
            opened_at: Utc::now(),
        };
    ...
    pub async fn total_value_usd(&self) -> Result<Decimal> {
        ...
        let total = positions.iter().fold(Decimal::ZERO, |acc, p| {
            acc + (p.amount * p.currency.usd_price_estimate())
        });
        Ok(total)
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_allocate_tracks_positions_and_total_value() {
        let optimizer = YieldOptimizer::new().await.unwrap();

        let position = optimizer
            .allocate(Currency::USDC, Decimal::from(1000), "Aave".to_string())
            .await
            .expect("allocation should succeed");

        assert_eq!(position.protocol, "Aave");
        assert_eq!(position.currency, Currency::USDC);
    ...
    #[tokio::test]
    async fn test_allocate_rejects_unknown_protocol() {
        let optimizer = YieldOptimizer::new().await.unwrap();
        let err = optimizer
            .allocate(
                Currency::USDC,
                Decimal::from(100),
                "UnknownProtocol".to_string(),
            )
            .await
            .expect_err("unknown protocol should fail");

        assert!(err.to_string().contains("unknown yield protocol"));
    }
}
