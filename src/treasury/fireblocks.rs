...
use super::{Transaction, TransactionStatus, TreasuryError, TreasuryResult};
...
impl FireblocksCustody {
    ...
    pub async fn new(config: FireblocksConfig) -> TreasuryResult<Self> {
        ...
        let secret = tokio::fs::read_to_string(&config.api_secret_path)
            .await
            .map_err(|e| TreasuryError::ConfigError(format!("Failed to read API secret: {}", e)))?;
    ...
    async fn ping(&self) -> TreasuryResult<()> {
        ...
        if response.status().is_success() {
            ...
            Err(TreasuryError::AuthenticationError(format!(
                "API test failed: {}",
                response.status()
            )))
        }
    ...
    pub async fn get_balances(&self) -> TreasuryResult<Vec<AssetBalance>> {
        let url = format!(
            "{}/v1/vault/accounts/{}",
            self.config.api_url, self.config.vault_account_id
        );
    ...
    pub async fn create_deposit_address(&self, asset_id: &str) -> TreasuryResult<DepositAddress> {
        let url = format!(
            ...
            self.config.api_url, self.config.vault_account_id, asset_id
        );
        ...
        info!(
            "Created deposit address for {}: {}",
            asset_id, address.address
        );
        Ok(address)
    }
