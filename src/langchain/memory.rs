...
impl ConversationMemory for VectorStoreMemory {
    ...
    async fn save(&self, human: &str, ai: &str) {
        self.retriever
            .add(&self.session_id, &Message::user(human))
            .await;
        self.retriever
            .add(&self.session_id, &Message::assistant(ai))
            .await;
    }

    async fn clear(&self) {
        // VectorRetriever does not support selective deletion — clearing requires
        // recreating the vector index, which is handled at the infrastructure level.
        tracing::warn!(
            session_id = %self.session_id,
            "VectorStoreMemory::clear() is a no-op: VectorRetriever does not support selective deletion"
        );
    }
}
...
impl ConversationMemory for SummaryMemory {
    ...
    async fn save(&self, human: &str, ai: &str) {
        ...
        // Periodic summary regeneration via LLM is not yet implemented.
        // Current behaviour: summary is only set at construction time.
        // When implemented, the LLM chain would condense recent_messages into
        // a new summary string stored in self.summary.
        tracing::warn!("SummaryMemory: periodic LLM-based summary update is not yet implemented");
    }
...
impl ConversationMemory for EntityMemory {
    async fn load(&self) -> Vec<Message> {
        ...
        vec![Message::system(format!("Known entities:\n{}", context))]
    }
