...
use super::{
    ...
    Chain, ChainContext, ChainError, ChainResult, ExecutionMetadata,
};
...
impl Chain for Agent {
    async fn run(&self, context: ChainContext) -> Result<ChainResult, ChainError> {
        ...
        let question = context.get("question").ok_or_else(|| {
            ChainError::ExecutionError("Missing 'question' in context".to_string())
        })?;
        ...
        let final_output = loop {
            ...
            if let Some((tool_name, tool_input)) = parse_action(&response) {
                ...
                messages.push(Message::user(format!("Observation: {}", result.output)));
...
fn parse_action(response: &str) -> Option<(String, String)> {
    ...
    match (action_idx, input_idx) {
        (Some(ai), Some(ii)) => {
            ...
            let input = lines[ii]
                .trim_start_matches("Action Input:")
                .trim()
                .to_string();
            Some((action, input))
        }
