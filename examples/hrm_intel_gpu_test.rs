...
use investor_os::hrm::{HRM, HRMConfig, InferenceResult};
...
fn main() {
    ...
    for (name, signals) in &scenarios {
        ...
        let trade = if result.should_trade(0.7) { "✅ YES" } else { "❌ NO" };
        
        println!("{:<15} {:>10.2} {:>12.2} {:>10?} {:>12}", 
            name, 
            result.conviction, 
            result.confidence,
            result.regime,
    ...
    println!("\n⚡ Batch Inference:");
    let batch: Vec<Vec<f32>> = scenarios.iter()
        .map(|(_, s)| s.clone())
        .collect();
    ...
    println!("   Processed {} signals in {:?}", batch_results.len(), elapsed);
    println!("   Average time per inference: {:?}", elapsed / batch_results.len() as u32);
